...
export default {
    ...
    // 订单支付页在被创建出来的时候判断有没有登录,如果登录了则去结算页,如果没登录则去登录页
    created(){
         // 获取数据 路由的数据
    ...
    .then((response)=>{
        ...
        // 此时的购买数量都为1 ,整合vuex中的购买数量到这里的数据中
        ...
        //给页面中标签的绑定数据
        //计算出总金额
        this.orderInfo.goodsAmount=parseInt(this.totalAmount)+parseInt(this.orderInfo.expressMoment);
        //赋值属性
     ...
     submitForm(formName) {
        ...
        if (valid) {
          ...
          .then(response=>{
            ...
            //跳转地址
            this.$router.push('/order/'+response.data.message.orderid);
          })
          .catch(err=>{
              console.log(err);
          })
